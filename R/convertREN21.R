...
# Obtaining the capacity factors (nur) values and associated maxproduction (maxprod) for Hydro, Wind, and Solar
    ...
    # For countries with targets but without maxprod and nur value for solar (in data_solar),
    # we hardcode
    # data_combined[c("KOR"), , "SolarPV"][, , "maxprod"]  <- as.vector(data_solar[c("JPN","GRC"), , "maxprod"][, , "spv"])
