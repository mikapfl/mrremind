...
calcHistorical <- function() {
  ...
  fe_proj <- fe_proj[, getYears(fe_proj, TRUE)[which(getYears(fe_proj, TRUE) <= 2100)], ] # get rid of periods after 2100
  ...
  EU_ReferenceScenario <- calcOutput("EU_ReferenceScenario", aggregate = FALSE)
  ...
  IEA_EVOutlook <- calcOutput("IEA_EVOutlook", aggregate = FALSE)
  ...
  # INNOPATHS data
  INNOPATHS <- calcOutput("INNOPATHS", aggregate = FALSE)
  ...
  UNFCCC <- UNFCCC[, seq(1986, 1989, 1), , invert = TRUE]
