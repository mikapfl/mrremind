...
readEurostat_EffortSharing <- function(subtype) {
   ...
   if (subtype == "target") {
     ...
     colnames(data) <- c("iso2", "country.name", "iso3", "2020", "2030", "flexibility_from_ETS_in_2030", "flexibility_from_land_use_in_2030")
     ...
     colnames(data) <- c("indic_eu", "iso2", "2005", "2006", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "target")
     iso2toiso3 <- c("LU" = "LUX", "SE" = "SWE", "DK" = "DNK", "FI" = "FIN", "DE" = "DEU",
     ...
     data <- na.omit(data[, c("iso3", "2005", "2006", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016")])
   } else {
     stop("Not a valid subtype!")
   }

   # data in wide format
   data <- melt(data, id.vars = c("iso3"), variable.name = "period", value.name = "value")  # melt requires library(reshape2)

   # converting to magpie object
   x <- as.magpie(data, spatial = 1, temporal = 2)
