...
toolSolarFunctionAggregate <- function(x, rel = NULL, weight = calcOutput("FE", aggregate = FALSE)[, "y2015", "FE|Electricity (EJ/yr)"]) {
  ...
  df.interpolate <- df.cuml %>%
    ...
    mutate(FLH =  na.approx(FLH, na.rm = FALSE), maxprod = na.approx(maxprod, na.rm = FALSE),
           area = na.approx(area, na.rm = FALSE), capacity = na.approx(capacity, na.rm = FALSE)) %>%
  ...
  out[, , "luse.spv"] <- dimSums(out, dim = 3.3, na.rm = TRUE)[, , "capacity"][, , "spv"] / dimSums(out, dim = 3.3, na.rm = TRUE)[, , "limitGeopot"][, , "spv"]
  out[, , "luse.csp"] <- dimSums(out, dim = 3.3, na.rm = TRUE)[, , "capacity"][, , "csp"] / dimSums(out, dim = 3.3, na.rm = TRUE)[, , "limitGeopot"][, , "csp"]
  # remove capacity
