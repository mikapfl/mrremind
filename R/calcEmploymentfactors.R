...
calcEmploymentfactors <- function(improvements, multiplier) {
  if (improvements == "None" || improvements == "CEEW" || improvements == "Dias" || improvements == "Rutovitz_aus" || improvements == "Solar_found" || improvements == "All") {
    ...
    if (multiplier == "Rutovitz") {
      ...
      x2_tmp[, 2050, ] <- 1
      x2_tmp <- time_interpolate(x2_tmp, c(2025, 2035, 2040, 2045), integrate_interpolated_years = TRUE)
      x2 <- x2_tmp
    ...
    if (multiplier == "own" || multiplier == "static") {
      ...
      y_gdpcc <- calcOutput("GDPpc", naming = "scenario", aggregate = FALSE)[, seq(2015, 2050, 5), "SSP2"] # SSP2 gdp per capita projections
    ...
    x4 <- calcOutput("CoalLabourProductivity", subtype = "Employment_factor", aggregate = FALSE)[, 2020, ]
    ...
    if (improvements == "Rutovitz_aus" || improvements == "All") {
      ...
      x9[, , "Wind onshore", pmatch = TRUE] <- c(2.8, 0.2)
      x9[, , "Solar|PV-utility", pmatch = TRUE] <- c(2.3, 0.1)
      x9[, , "Solar|PV-rooftop", pmatch = TRUE] <- c(5.8, 0.2)
      x9[, , "Storage|Battery", pmatch = TRUE] <- c(4.7, 1.2)
      x9[, , "Hydro-large", pmatch = TRUE] <- c(6, 0.15)
    ...
    x1_tmp <- new.magpie(getRegions(x1), seq(2015, 2050, 5), getNames(x1), fill = 1)
     ...
     if (multiplier == "Ram") {
x2[oecd_con, , ] <- 1
} # EF for OECD countries does not change
...
wt <- x1
y <- readSource("ConferenceBoard")
wt[, , c("Solar|PV", "Storage"), pmatch = TRUE] <- gen[, "y2019", "Generation|Solar (TWh)"]
wt[, , "Hydro", pmatch = TRUE] <- gen[, "y2019", "Generation|Hydro (TWh)"]
...
wt[, , "Wind", pmatch = TRUE] <- gen[, "y2019", "Generation|Wind (TWh)"]
wt[, , "Nuclear", pmatch = TRUE] <- gen[, "y2019", "Generation|Nuclear (TWh)"]
...
wt[, , "Biomass.Fuel_supply"] <- y[, "y2019", "Employment in agriculture"]
 ...
 # gdp <-   calcOutput("GDP",   years=2020, aggregate = FALSE)
 # gdp <- gdp[,,"gdp_SSP2"]
