#' Calculate regional Theil-TRUE index
#'

#' To calculate the regional Theil-TRUE index (= correction to welfare function for a lognormal income distribution) we do the following:
#' (1) convert country-level Gini coefficients to Theil (2) calculate contribution to Theil-TRUE index that includes both between-countries and within-country inequality
...
#'
#' NB 2: the result of calcOutput('Theil', aggregate = FALSE), is NOT the country Theil-TRUE, but the unweighted contribution from a given country to the regional value.
#'
...
calcTheil <- function() {
  ...
  return(list(x = contribTheilT, weight = s_i, unit = "-", description = "aggregated: Theil-TRUE index, not-aggregated: unweighted contribution to Theil-TRUE"))
}

##  helper functions.
TheilT.from.sigma <- function(sigma) {
  # Theil TRUE coefficient for lognormal distribution
  TheilT <- sigma^2 / 2.
  return(TheilT)
}
