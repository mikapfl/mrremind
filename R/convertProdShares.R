...
convertProdShares <- function(x) {
  ...
  x <- readSource(type = "ProdShares", convert = FALSE)
  ...
  wt <- calcOutput(type = "GDP", aggregate = FALSE)
  ...
  wind <- x[, , "wind"]
  wind["Other", , ] <- 1 - dimSums(wind, dim = 1, na.rm = TRUE)
  ...
  y <- x[c("Europe", "Other"), , invert = TRUE]
