...
calcGEA2012 <- function(subtype, datatype) {
  ...
  if (subtype == "coal") {
    ...
    if (datatype == "exportbound") {
      ...
      x <- new.magpie(regionsH12, ttot, names = paste(rep("peoil", nscen), ssps, sep = "."), fill = NA)
      x["MEA", "y2020", "peoil"][, , c("SSP1", "SSP2")] <- xport2005MEA * 1.2877
      x["MEA", "y2025", "peoil"][, , c("SSP1", "SSP2")] <- xport2005MEA * 1.40094
      x["MEA", "y2030", "peoil"][, , c("SSP1", "SSP2")] <- xport2005MEA * 1.52414
      x["MEA", "y2035", "peoil"][, , c("SSP1", "SSP2")] <- xport2005MEA * 1.65817
      x["MEA", "y2020", "peoil"][, , "SSP5"] <- xport2005MEA * 1.2877 * 1.25
      x["MEA", "y2025", "peoil"][, , "SSP5"] <- xport2005MEA * 1.40094 * 1.3
