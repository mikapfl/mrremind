...
calcFEdemand <- function(subtype = "FE") {
    ...
    addSDP_transport <- function(rmnditem) {
      ...
      pop <- pop[CJ(region = pop$region, year = yrs, scenario = pop$scenario, unique = TRUE),
    ...
    if (subtype %in% c("FE", "EsUeFe_in", "EsUeFe_out", "FE_buildings",
      ...
      buildings <- time_interpolate(buildings, interpolated_year = fill_years, integrate_interpolated_years = TRUE, extrapolation_type = "constant")
    ...
    if (subtype %in% c("FE", "FE_buildings", "UE_buildings", "FE_for_Eff", "UE_for_Eff", "ES")) {
      ...
      mapping <- rbind(
        ...
        mapping <- read.csv2(mapping_path, stringsAsFactors = FALSE)
    }
    #----- PROCESS DATA ------------------
