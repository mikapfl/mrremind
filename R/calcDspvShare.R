...
calcDspvShare <- function(subtype) {
  ...
  if (subtype == "expert") {
    ...
    share_spv[, 2030, ] <- 0.3
    share_spv <- time_interpolate(share_spv, 2025, integrate_interpolated_years = TRUE)

    # for IND and Japan these are 40% in 2030.
    share_tmp <- share_spv[c("IND", "JPN"), c(2015, 2020, 2030), ]
    share_tmp[, 2030, ] <- 0.4
    share_tmp <- time_interpolate(share_tmp, 2025, integrate_interpolated_years = TRUE)
  ...
  if (subtype == "irena") {
    ...
    share_spv <- time_interpolate(share_spv, seq(2020, 2050, 5), integrate_interpolated_years = TRUE)
  }
  ...
  # countries with longest coastline have 30% wind share in 2050. Interpolated linearly.
    ...
    share_top_20 <- time_interpolate(share_top_20, seq(2020, 2050, 5), integrate_interpolated_years = TRUE)
    ...
    share_rem[, 2050, ] <- 0.1
    share_rem <- time_interpolate(share_rem, seq(2025, 2045, 5), integrate_interpolated_years = TRUE)
  ...
  share_tmp[, "y2050", ] <- 0.2
  share_tmp <- time_interpolate(share_tmp, seq(2025, 2045, 5), integrate_interpolated_years = TRUE)
