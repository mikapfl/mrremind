...
calcCoalLabourProductivity <- function(subtype) {
  Year <- NULL
  emp <- calcOutput("ILO", subtype = "all", aggregate = FALSE)[, , "Coal and Lignite"]
...
if (subtype == "Employment_factor") {
  ...
  #
  # ef_fut <- time_interpolate(ef_fut,interpolated_year = seq(  2035,2045,5),integrate_interpolated_years = TRUE)
  ...
  x_final <- toolCountryFill(x_final, fill = 0)
