...
calcCapacityFactorHist <- function(subtype) {
  ...
  # check data
  max(cf_realworld[, , "hydro"], na.rm = TRUE)
  max(cf_realworld[, , "wind"], na.rm = TRUE) # INF spm, >1 AZE
  max(cf_realworld[, , "spv"], na.rm = TRUE)
  max(cf_realworld[, , "csp"], na.rm = TRUE)
  max(cf_realworld[, , "bioigcc"], na.rm = TRUE) #>1 CHL, JPN, POL
  max(cf_realworld[, , "geohdr"], na.rm = TRUE)
  ...
  return(list(x = cf_realworld, weight = hist_gen,
    ...
    # check data
    max(cf_year[, , "hydro"], na.rm = TRUE)
    max(cf_year[, , "wind"], na.rm = TRUE) # INF spm, >1 AZE
    max(cf_year[, , "windoff"], na.rm = TRUE)
    max(cf_year[, , "spv"], na.rm = TRUE)
    max(cf_year[, , "csp"], na.rm = TRUE)
    max(cf_year[, , "bioigcc"], na.rm = TRUE) #>1 CHL, JPN, POL
    max(cf_year[, , "geohdr"], na.rm = TRUE)
