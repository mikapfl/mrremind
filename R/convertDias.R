...
convertDias <- function(x, subtype) {
  if (subtype == "Employment factors") {
    # x <- readSource(type = "Dias", subtype = "Employment factors", convert = FALSE) # EFs for coal and coal mining

    getRegions(x) <- toolCountry2isocode(getRegions(x))
    x <- toolCountryFill(x, fill = NA)
  }
  if (subtype == "Employment") {
    # x <- readSource(type = "Dias", subtype = "Employment", convert = FALSE) # EFs for coal and coal mining
