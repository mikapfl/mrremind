...
calcEnergyEffPaths <- function() {
  ...
  #----- READ-IN DATA ------------------
 fe <- calcOutput("FEdemand", subtype = "FE_for_Eff", aggregate = FALSE)
 ue <- calcOutput("FEdemand", subtype = "UE_for_Eff", aggregate = FALSE)
 #----- PARAMETERS ------------------
 t0 <- 2005
 ...
 output <- output %>% as.quitte() %>%
   ...
   select(-"tmp") %>% ungroup() %>%
   interpolate_missing_periods_(list(period = getYears(output, TRUE)), expand.values = TRUE) %>%
   removeColNa() %>%
