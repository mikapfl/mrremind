...
readEDGETransport <- function(subtype = "logit_exponent") {
  ...
  ## load the data from EDGE-TRUE
  EDGETrData <- calcOutput("EDGETrData", aggregate = FALSE)
  EDGETrData_all <- list()
  ...
  create_copy_demscens <- function(dt) {
    ...
    ## Workaround for NAVIGATE: copy-create demand scenarios which we do not supply by EDGE-TRUE
    dt <- rbind(dt,
  ...
  switch(subtype,
         "logit_exponent" = {
           ## do not call with convert=TRUE, there is only global data!
         ...
         "demISO" = {
           ...
           tmp <- merge(tmp, map, by = "all_in", all = TRUE, allow.cartesian = TRUE)
